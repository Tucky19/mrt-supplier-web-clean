...
async function ConversionInsights({ days }: { days: number }) {
  const data = await fetchJSON<ConversionResp>(
    `${process.env.NEXT_PUBLIC_SITE_URL ?? ""}/api/admin/analytics/conversion?days=${days}`
  );
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Table
        ...
        headers={["Part No", "Clicks", "RFQ", "Rate"]}
        rows={data.productConversion.slice(0, 15).map((row) => [
          row.partNo,
          row.clicks,
          row.rfqs,
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Suspense fallback={<div>Loading conversion insights...</div>}>
          <ConversionInsights days={days} />
        </Suspense>
      </div>
    </main>
  );
